 ...
 * @author X
 */
public class QiyeguanliServlet extends HttpServlet {
	...
	Qiye qiye = new Qiye();
		...
		try {
			...
			if("admin".equals((String)currentUserType)) {
				...
				request.setAttribute("mainPage", "admin/qiyechakan.jsp");
		...
		try {
			con = dbUtil.getCon();
			qiyeDao.qiyeDelete(con, comid);
			request.getRequestDispatcher("q?action=list").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	private void qiyeSave(HttpServletRequest request,
		...
		Qiye qiye = new Qiye(quancheng,jiancheng,xindai,descrip,city,email,"",qh,Integer.parseInt(zaixianNum),1);
		...
		try {
			...
			int saveNum = 0;
			if(StringUtil.isNotEmpty(comid)) {
				saveNum = qiyeDao.qiyeUpdate(con, qiye);
			}  else {
				saveNum = qiyeDao.qiyeAdd(con, qiye);
			}
			if(saveNum > 0) {
				request.getRequestDispatcher("q?action=list").forward(request, response);
			} else {
				...
				request.setAttribute("mainPage", "admin/qiyeSave.jsp");
				request.getRequestDispatcher("mainAdmin.jsp").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
