 ...
 * @author X
 */
public class JianlisServlet extends HttpServlet {
   ...
   private static final long serialVersionUID = 1L;
	...
	Jianli jianli = new Jianli();
		...
		try {
			con=dbUtil.getCon();
				...
				request.setAttribute("mainPage", "student/jianli.jsp");
	...
	private void jianliSave(HttpServletRequest request,
		...
		try {
			...
			if(saveNum > 0) {
				request.getRequestDispatcher("jstu?action=list").forward(request, response);
			} else {
				request.setAttribute("jianli", jianli);
				request.setAttribute("error", "����ʧ��");
				request.setAttribute("mainPage", "student/jianliSave.jsp");
				request.getRequestDispatcher("mainStudent.jsp").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
