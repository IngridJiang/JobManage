...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Qiyezhanghao currentQiyezhanghao = null;
			...
			} else if("q".equals(userType)) {
				Qiyezhanghao q = new Qiyezhanghao(userName, password);
				currentQiyezhanghao = userDao.Login(con, q);
				if(currentQiyezhanghao == null) {
					request.setAttribute("qiyezhanghao", q);
					request.setAttribute("error", "用户名或密码错误");
					request.getRequestDispatcher("login.jsp").forward(request, response);
				} else {
					if("remember-me".equals(remember)) {
					...
					session.setAttribute("currentUserType", "qiyezhanghao");
					session.setAttribute("currentUser", currentQiyezhanghao);
					request.setAttribute("mainPage", "qiye/blank.jsp");
